...
func Test_Handler_GoRoutines_FirstFails(t *testing.T) {
	t.Parallel()
	order := New("order", Settings{Timeout: 2 * time.Second})

	handlerB, ctxB, doneB := goroutine.New("B", goroutine.Settings{Timeout: time.Nanosecond})
	go functionB(ctxB, doneB)
	order.Append(handlerB)

	handlerA, ctxA, doneA := goroutine.New("A", goroutine.Settings{})
...
func Test_Handler_GoRoutines_FirstFailsCritical(t *testing.T) {
	...
	handlerB, ctxB, doneB := goroutine.New("B", goroutine.Settings{Timeout: time.Nanosecond, Critical: true})
	go functionB(ctxB, doneB)
	order.Append(handlerB)
